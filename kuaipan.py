...
def load(filename):
		with open(filename, 'rt')as f:
			...
			return KuaipanFile(
				rs.get('consumer_key'),
				rs.get('consumer_secret'),
				rs.get('oauth_token'),
				rs.get('oauth_token_secret'))
		
def auth_me(filename):
		CONSUMER_KEY = 'xcNBQcp5oxmRanaC'
		CONSUMER_SECRET = 'ilhYuLMWpyVDaLm4'
		ROOT = 'app_folder'
		kuaipan_oauth = KuaipanOAuth(CONSUMER_KEY, CONSUMER_SECRET)
...
if __name__ == '__main__':
	...
	try:
		kf = load(CACHED_KEYFILE)
	except:
		kf = auth_me(CACHED_KEYFILE)
	print kf.account_info()
	#print kf.metadata('/')
	#print kf.upload_file('tempfile0', kuaipan_path='tempfile', ForceOverwrite=True)
	print kf.metadata('/b/b037fcb7642e1ad16d06770b68b84238df5f8938')
